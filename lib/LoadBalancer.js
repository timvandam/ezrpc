...
class LoadBalancer extends Server {
   ...
   * @param {ClientOptions} [options] - options to provide the created clients
   * @param {ReconnectStrategy} [options.reconnectStrategy=Client.ReconnectStrategies.Linear(1)] - the reconnect strategy to use
   * @param {Number} [options.maxReconnectAttempts=-1] - the amount of times to try to reconnect
   */
  constructor (servers, port, options = {}) {
    options = {
      reconnectStrategy: Client.ReconnectStrategies.Linear(1),
      maxReconnectAttempts: -1,
      ...options
    }
    if (!Array.isArray(servers)) throw new InvalidArgumentException('You must provide an array of servers!')
    if (!servers.length) throw new InvalidArgumentException('You must provide at least one server!')
    for (let i = 0; i < servers.length; i++) {
      const { host, port } = servers[i]
      if (!host) throw new InvalidArgumentException('You must provide a server host!')
      if (typeof host !== 'string') throw new InvalidArgumentException('Server host must be a string!')
      if (!port) throw new InvalidArgumentException('You must provide a server port!')
      if (typeof port !== 'number') throw new InvalidArgumentException('Server port must be numeric!')
    }
    super(port)
    this.clients = servers.map(({ host, port }) => new Client(host, port, options))
