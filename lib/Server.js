      ...
      reader.on('message', async data => {
        try {
          ...
          if (!method) {
            ...
            socket.write(new Callback(messageId, null, new InvalidArgumentException('No such method: ' + methodName + '(..)')).getBuffer())
            return
          }
          debug('calling %s', methodName)
          method(...args)
            .then(response => socket.write(new Callback(messageId, response, null).getBuffer()))
            .catch(error => socket.write(new Callback(messageId, null, error).getBuffer()))
        } catch (error) {
