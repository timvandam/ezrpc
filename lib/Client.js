const { InvalidArgumentException, ConnectionError, InvalidStateException } = require('ezerrors')
...
class Client extends EventEmitter {
  ...
  constructor (host, port, {
    ...
    if (!host || typeof host !== 'string') throw new InvalidArgumentException('Provide a host!')
    if (!port || typeof port !== 'number') throw new InvalidArgumentException('Provide a numeric port!')
    if (typeof maxReconnectAttempts !== 'number') throw new InvalidArgumentException('Provide a numeric maxReconnectAttempts!')
    if (typeof reconnectDelay !== 'number') throw new InvalidArgumentException('Provide a numeric reconnectDelay!')
    if (typeof reconnectStrategy !== 'function') throw new InvalidArgumentException('ReconnectStrategy must be a function!')
    super()
    this.options = {
      host,
      port: parseInt(port)
  ...
  setUp () {
    ...
    this.socket.on('close', async () => {
      ...
      try {
        ...
        this.destroyed = true
        this.reconnecting = false
        this.emit('error', new ConnectionError('Could not reconnect to the server'))
      }
    })
  }
  ...
  connect () {
    return new Promise((resolve, reject) => {
      ...
      function errorListener (error) {
        reconnecting ? reject(error) : resolve()
  ...
  call (methodName, ...args) {
    ...
    if (this.destroyed) return Promise.reject(new InvalidStateException('This instance has been destroyed'))
    if (this.reconnecting) return Promise.reject(new InvalidStateException('Not connected to the server. Attempting to reconnect...'))
...
Client.ReconnectStrategies = class {
  ...
  static Exponential (base = 2) {
    if (typeof base !== 'number') throw new InvalidArgumentException('Base must be numeric!')
  ...
  static Linear (slope = 1) {
    if (typeof slope !== 'number') throw new InvalidArgumentException('Slope must be numeric!')
