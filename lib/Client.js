...
class Client extends EventEmitter {
   ...
   * @param {Number} [opts.maxReconnectAttempts=5] - the amount of times to attempt to reconnect to the server on disconnect
   * @param {Number} [opts.reconnectDelay=1000] - how many ms to wait before the first reconnect attempt (doubles every attempt)
   * @param {ReconnectStrategy} [opts.reconnectStrategy=Client.ReconnectStrategies.Exponential(2)] - function that computes the time between reconnects
   */
  constructor (host, port, {
    ...
    if (typeof reconnectStrategy !== 'number') throw new Error('Provide a numeric reconnectDelay!')
    super()
    this.options = {
      host,
      port: parseInt(port)
