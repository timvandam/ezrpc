...
describe('feed works', () => {
  ...
  it('when providing a complete message', () => {
    reader.feed(Buffer.from('0100000041', 'hex')) // msg length = 1 = 0x0100 (LE 2 bytes). Message = 0x41 = hex for A
    expect(message).toHaveBeenCalled()
    expect(message).toHaveBeenLastCalledWith(Buffer.from('A'))
  })

  it('when providing partial messages', () => {
    reader.feed(Buffer.from('02000000', 'hex')) // 2 bytes
  ...
  it('when providing multiple messages', () => {
    ...
    reader.feed(Buffer.from('01000000410100000042', 'hex'))
    reader.removeAllListeners('message')
